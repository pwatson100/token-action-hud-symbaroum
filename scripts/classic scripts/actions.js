...
export class ActionHandlerSymbaroum extends ActionHandler {
	...
	_getMysticalPowers(actor, tokenId) {
		let filteredItems = actor.items.filter((item) => item.type === "mysticalPower" && item.system?.hasScript);
	...
	_getTraits(actor, tokenId) {
		let filteredItems = actor.items.filter((item) => item.type === "trait" && item.system?.hasScript);
	...
	_getWeapons(actor, tokenId) {
		let filteredItems = actor.items.filter((item) => item.type === "weapon" && item.system?.state === "active");
	...
	_getArmors(actor, tokenId) {
		...
		armorsCategory.actions = this._produceMap(tokenId, items, "armor");
	...
	_getAbilities(actor, tokenId) {
		let filteredItems = actor.items.filter((item) => item.data?.type === "ability" && item.system?.script);
